...
{ 
  ...
  cv::cvtColor(curr_image[0], lab, CV_BGR2Lab);
...
/*    for(int i = 0; i < past_images.size(); i ++)
  ...
  dist = cv::Scalar((lab.at<cv::Vec3b>(200,150))[0], (lab.at<cv::Vec3b>(200,150))[1], (lab.at<cv::Vec3b>(200,150))[2], 0) - mean;
  dist = cv::Scalar(dist[0]/sum[0], dist[1]/sum[1], dist[2]/sum[2], 0);
 ...
 /*compare blue image*/
  ...
  cv::cvtColor(curr_image, lab, CV_BGR2Lab);
  dist = cv::Scalar((lab.at<cv::Vec3b>(326,187))[0], (lab.at<cv::Vec3b>(326,187))[1], (lab.at<cv::Vec3b>(326,187))[2], 0) - mean;
  dist = cv::Scalar(dist[0]/sum[0], dist[1]/sum[1], dist[2]/sum[2], 0);
...
void LedFinder::CloudDifferenceTracker::differenceImage(std::vector<cv::Mat>& curr_images, std::vector<cv::Mat>& past_images, cv::Mat& led, cv_bridge::CvImagePtr cv, pcloud_ cl)
{
  ...
  channels.resize(3);cv::Mat lab3;
  cv::cvtColor(diff,lab3, CV_BGR2Lab);
  cv::split(lab3,channels);
  double *min;
  double *max;
  cv::Point *minpt(new cv::Point);
  cv::Point *maxpt(new cv::Point);
  cv::minMaxLoc(channels[0], min, max, minpt, maxpt, cv::Mat());
  std::cout<<*maxpt<<" "<< max<<std::endl;

  debug_img(im->image, "/tmp/mean/past_", 0,0,0);
  debug_img(cv->image, "/tmp/mean/curr_", 0,0,0);
  debug_img(diff, "/tmp/mean/diff_", 0,0,0);
  debug_img(channels[0], "/tmp/mean/diflatyer_", 0,0,0);
...
/*  cv::Mat sum_image = cv::Mat::zeros(past_images[0].size(), CV_32SC3);
  ...
  cv::Mat mean_image = sum_image;
  sum_image = cv::Mat::zeros(past_images[0].size(), CV_32SC3);
  ...
  cv::Mat std_dev_FC3, std_dev_SC3,tmp;
  ...
  cv::Mat lab_curr;
  cv::cvtColor(curr_images[0], lab_curr, CV_BGR2Lab);
  lab_curr.convertTo(curr_SC3, CV_32SC3);
  cv::absdiff(curr_SC3, mean_image,tmp);
  cv::divide(tmp, std_dev_SC3, dist);

  cv::Mat dist_8UC1;
  cv::Rect rect = cv::Rect(100,100, 540, 380);
