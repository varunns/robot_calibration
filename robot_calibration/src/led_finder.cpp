...
//    localDebugImage((tracker_in->rgb_image)[i], "/tmp/mean/diff_");
 ...
 // localDebugImage(tracker_in->diff_images[0], "/tmp/mean/img_");
  ...
  if(max_contour.size () > 0 )
  {
    ...
    float total = 0;
    for( size_t i = bounds.tl().x - 8 ; i < bounds.tl().x - 8 + 16; i++)
    {
      for( size_t j = bounds.tl().y - 8; j < bounds.tl().y - 8 + 16; j++)
      {
    ...
    pcl::PointXYZRGB pt3;
    cv::rectangle(tracker_in->diff_images[1], cv::Rect(round(center_x) -4, round(center_y) -4, 8, 8), cv::Scalar(0,0,255), 1, 8);
