...
/*  std::cout<<"&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&"<<std::endl;
  ...
  for( int i = 317; i < 337; i++)
  {
    for( int j = 233; j < 253; j++)
    {
      for( int k = 0; k < tracker_in->pclouds.size(); k++)
      {
        ...
        dist = std::sqrt(pow((pt3.x - tracker_in->pt3d.x),2)+pow((pt3.y - tracker_in->pt3d.y),2)+pow((pt3.z - tracker_in->pt3d.z),2));
  ...
  {
    bounds = cv::boundingRect(max_contour);
  }
...
/*  cv::rectangle(tracker_in->diff_images[5], cv::Rect(max_point.x - 8, max_point.y - 8, 16, 16), cv::Scalar(0,255,0), 1,8);
  ...
  {
    for( int k = max_point.y - 8; k < max_point.y + 16; k++)
    {
      pcl::PointXYZRGB pt3;
      cv::Point pt = cv::Point(i,k);
...
{
  //Popuplating the TrackerContour pointer for first time with id 
  //and the respective actual 3d location according to the transform, only once
...
void LedFinder::CloudDifferenceTracker::convert2CvImagePtr(std::vector<pcloud_>& pcl_cloud, std::vector<cv_bridge::CvImagePtr>& cv_ptr)
{
  ...
  for(size_t i = 0; i < pcl_cloud.size(); i++)
  {
    cv_ptr[i].reset(new cv_bridge::CvImage);
      ...
      {
        if(pcl_cloud[i]->points[j].z < 0.2 ||pcl_cloud[i]->points[j].z > 1.0 || isnan(pcl_cloud[i]->points[j].z))
        {
...
/*    pcl::PointCloud<pcl::PointXYZRGB>::Ptr passed_cloud(new pcl::PointCloud<pcl::PointXYZRGB>);
    ...
    try
    {
      ...
      debug_img(cv_ptr[i]->image, "/tmp/mean/pass_",0,0,0);
    }
